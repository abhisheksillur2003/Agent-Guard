...
Phase 12 adds `agentguard_sdk.AgentGuardClient`, an asynchronous typed client for agent applications. Its `guarded_execute` method evaluates a tool request and calls the execution endpoint only after an explicit `allow` decision. Denials and approval requirements return typed stopped states. Network failures, malformed responses, and API errors raise SDK errors and never trigger a local fallback.

The included Ollama example lets a local model propose an operation and JSON arguments while AgentGuard remains the only authority that can execute the registered adapter:
...
```text
agentguard_sdk/         Typed Python client for agent applications
backend/
...
examples/                 Runnable guarded agent examples
